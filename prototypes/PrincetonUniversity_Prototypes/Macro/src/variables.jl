...
function add_operation_variables!(g::AbstractStorage, model::Model)
    ...
    for P in time_subperiods
        @constraint(model,[t in P[2:end]], storage_level(g)[t] == (1+self_discharge(g))*storage_level(g)[t-1] + efficiency_discharge(g)*injection(g)[t] - efficiency_charge(g)*withdrawal(g)[t])
        t_start = first(P);
        t_end = last(P);
        @constraint(model,storage_level(g)[t_start] == (1+self_discharge(g))*storage_level(g)[t_end] + efficiency_discharge(g)*injection(g)[t] - efficiency_charge(g)*withdrawal(g)[t])
    end
