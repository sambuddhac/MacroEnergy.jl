...
Base.@kwdef mutable struct Node{T} <: AbstractNode{T}
    ...
    constraints ::Vector{AbstractTypeConstraint}=[EnergyBalanceConstraint{T}(),MaxNonServedEnergyConstraint{T}()]
end
...
function add_operation_variables!(n::AbstractNode, model::Model)
    ...
    n.operation_vars[:non_served_energy] = @variable(model, [t in time_interval(n)], lower_bound = 0.0, base_name = "vNSE_$(commodity_type(n))_$(node_id(n))")

    n.operation_expr[:net_energy_production] =  @expression(model,[t in time_interval(n)], 0*model[:vREF]);
