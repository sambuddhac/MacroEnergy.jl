...
function generate_decomposed_system(system_full::Vector{Union{AbstractAsset,Edge,Node}},subperiod_indices::Vector{Int64})

    system_decomp = Dict{Int64,Vector{Union{AbstractAsset,Edge,Node}}}();
    
    for w in subperiod_indices
        system_decomp[w] = deepcopy(system_full)
        for y in system_decomp[w]
...
function generate_benders_data(system::Vector{Union{AbstractAsset,Edge,Node}},timedata::Dict{Symbol, TimeData})

    subperiod_indices = collect(eachindex(timedata[:Electricity].subperiods));
