...
balance_data(e::AbstractEdge,v::AbstractVertex,i::Symbol) = isempty(balance_data(v,i)) ? 1.0 : balance_data(v,i)[id(e)];
...
function planning_model!(e::AbstractEdge, model::Model)
    ...
    if has_planning_variables(e)
        ...
        e.new_capacity= @variable(
            model,
            lower_bound = 0.0,
            base_name = "vNEWCAP_$(id(e))"
        )

        e.ret_capacity = @variable(
            model,
            lower_bound = 0.0,
            base_name = "vRETCAP_$(id(e))"
        )
...
function operation_model!(e::EdgeWithUC, model::Model)
    ...
    if !e.unidirectional
       error("UC is available only for unidirectional edges, set edge $(id(e)) to be unidirectional")
       return nothing
    end
