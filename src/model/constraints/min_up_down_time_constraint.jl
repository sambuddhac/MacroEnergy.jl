...
function add_model_constraint!(
    ...
    if min_down_time(e) > minimum(length.(subperiods(e)))
        ...
        capacity(e)/capacity_size(e) - ucommit(e,t) >= sum(ushut(e,s) for s in [timestepbefore(t,h,subperiods(e)) for h in 0:min_down_time(e)-1])
        )
    end
...
function add_model_constraint!(
    ...
    if min_up_time(e)>minimum(length.(subperiods(e)))
        ...
        ct.constraint_ref = @constraint(model,
        [t in time_interval(e)],
        ucommit(e,t) >= sum(ustart(e,s) for s  in [timestepbefore(t,h,subperiods(e)) for h in 0:min_up_time(e)-1])
        )
    end
