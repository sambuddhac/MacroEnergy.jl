...
function test_writing_output()
    ...
    @testset "get_optimal_vars Tests" begin
        result = get_optimal_vars(mock_edges, capacity, obj_asset_map)
        ...
        result = get_optimal_vars(Edge{Electricity}[edge_between_nodes], (new_capacity))
        ...
        result = get_optimal_vars(Storage[storage], new_capacity_storage, Dict{Symbol, Base.RefValue{<: AbstractAsset}}(:storage1 => asset_ref2))
        @test length(result) == 1
        @test result[1].commodity == :Electricity
    ...
    @testset "get_optimal_vars_timeseries Tests" begin
        ...
        result = get_optimal_vars_timeseries(storage, storage_level, Dict{Symbol, Base.RefValue{<: AbstractAsset}}(:storage1 => asset_ref2))
        @test length(result) == 3
        for i = 1:3
            check_output_row(result[i], :Electricity, :storage_level, :storage1, :asset2, :storage1, :Battery, :storage_level, missing, 1, i, i, :MWh)
        end
        result = get_optimal_vars_timeseries(storage, tuple(storage_level), Dict{Symbol, Base.RefValue{<: AbstractAsset}}(:storage1 => asset_ref2))
        @test length(result) == 3
        for i = 1:3
            check_output_row(result[i], :Electricity, :storage_level, :storage1, :asset2, :storage1, :Battery, :storage_level, missing, 1, i, i, :MWh)
        end
        ...
        result = get_optimal_vars_timeseries([node1, node2], tuple(max_non_served_demand))
        @test length(result) == 6
        for i = 1:6
            check_output_row(result[i], :Electricity, :max_non_served_demand, i <= 3 ? :node1 : :node2, i <= 3 ? :node1 : :node2, i <= 3 ? :node1 : :node2, Symbol("Node{Electricity}"), :max_non_served_demand, missing, 1, (i-1) % 3 + 1, i-1, :MWh)
        end
